...
namespace calibration{
  ...
  enum class StochasticEDFMethod {
    Woodbury,
    Cholesky
  } ; 
  ...
  public:
    // constructor
    StochasticEDF(const Model& model, std::size_t r, std::size_t seed, StochasticEDFMethod method = StochasticEDFMethod::Woodbury)
      : model_(model), r_(r), seed_(seed), method_(method) {}
    StochasticEDF(const Model& model, std::size_t r, StochasticEDFMethod method = StochasticEDFMethod::Woodbury)
	...
	init_ = true; // never reinitialize again
      ...
      if(!model_.hasCovariates()){ // nonparametric case
        ...
        if (method_ == StochasticEDFMethod::Woodbury) {
          ...
          // sol = (model_.A() + model_.U()*(model_.invXtWX().reconstructedMatrix())*model_.V()).llt().solve( model_.PsiTD()*model_.lmbQ(model_.Psi()) ) ; 
          sol = SMW<>().solve(model_.invA(), model_.U(), model_.XtWX(), model_.V(), Bs_);
        }
