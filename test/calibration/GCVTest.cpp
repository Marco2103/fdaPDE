...
//    GCV optimization: grid exact
//  */
// TEST(GCV_STRPDE, Test1_Laplacian_NonParametric_GeostatisticalAtNodes_GridExact) {
...
//   CSVReader<double> reader{};
//   CSVFile<double> yFile; // observation file
//   yFile = reader.parseFile(path_test + "/dati_gaussiani_alpha_50/z.csv");
//   DMatrix<double> y = yFile.toEigen();

//   // set model data
//   BlockFrame<double, int> df;
//   df.stack(OBSERVATIONS_BLK, y);    // ATT: qui per ST c'è .stack (non .insert ) !!!
...
//   // std::vector<SVector<1>> lambdat;
//   // for(double x = -6.0; x <= -2.0; x +=0.25) lambdat.push_back(SVector<1>(std::pow(10,x)));

//   std::vector<SVector<2>> lambdas_t;
//   for(double x = -3.0; x <= -2.0; x +=0.25) lambdas_t.push_back(SVector<2>(std::pow(10,x), std::pow(10,x)));
...
//   // ScalarField<1, decltype(GCV)> obj(GCV);
//   // opt.optimize(obj, lambdas, lambdat); // optimize gcv field
//   // SVector<1> best_lambda = opt.optimum();
...
//   // Save GCV scores
//   std::ofstream fileGCV_scores(path_test + "/dati_gaussiani_alpha_50/GCV/GCV_scores_STRPDE.csv");
...
//   // Save edf
//   std::ofstream file_edf(path_test + "/dati_gaussiani_alpha_50/GCV/GCV_edf_STRPDE.csv");
//   for(std::size_t i = 0; i < GCV.values().size(); ++i) 
...
//    GCV optimization: grid exact
//  */
// TEST(GCV_STRPDE, Test1_Laplacian_NonParametric_GeostatisticalAtNodes_GridStoch) {
...
//   std::vector<SVector<2>> lambdas_t;
//   for(double x = -2.5; x <= -1.5; x +=0.25) lambdas_t.push_back(SVector<2>(std::pow(10,x), std::pow(10,x)));
...
//   // Save GCV scores
//   std::ofstream fileGCV_scores(path_test + "/alpha_" + alpha_string + "/GCV/GCV_scores_STRPDE.csv");
//   for(std::size_t i = 0; i < GCV.values().size(); ++i) 
//     fileGCV_scores << std::setprecision(16) << std::sqrt(GCV.values()[i]) << "\n" ; 
...
//   // Save edf
//   std::ofstream file_edf(path_test + "/alpha_" + alpha_string + "/GCV/GCV_edf_STRPDE.csv");
//   for(std::size_t i = 0; i < GCV.values().size(); ++i) 
//     file_edf << std::setprecision(16) << std::sqrt(GCV.edfs()[i]) << "\n" ; 
...
//  */
// TEST(STRPDE, Test2_Laplacian_SemiParametric_GeostatisticalAtLocations_Separable_Monolithic_GridExact) {
...
//   // define lambdas_t vector
//   std::vector<SVector<2>> lambdas_t;
//   for(double x = -1.5; x <= -2.25; x +=0.25) lambdas_t.push_back(SVector<2>(std::pow(10,x), std::pow(10,x)));
...
//    GCV optimization: grid exact
//  */
// TEST(GCV_SQRPDE, Test1_Laplacian_NonParametric_GeostatisticalAtNodes_GridExact) {
...
//   std::vector<SVector<2>> lambdas_t;
//   for(double x = -5.75; x <= -4.75; x +=0.25) lambdas_t.push_back(SVector<2>(std::pow(10,x), std::pow(10,x)));
...
//   // Save Lambda opt
//   std::ofstream fileLambdaopt(path_test + "/alpha_" + alpha_string + "/GCV/Exact/LambdaCpp.csv");
...
//   // Save GCV scores
//   std::ofstream fileGCV_scores(path_test + "/alpha_" + alpha_string + "/GCV/Exact/GCV_scores.csv");
//   for(std::size_t i = 0; i < GCV.values().size(); ++i) 
//     fileGCV_scores << std::setprecision(16) << std::sqrt(GCV.values()[i]) << "\n" ; 
...
//   // Save edf
//   std::ofstream file_edf(path_test + "/alpha_" + alpha_string + "/GCV/Exact/GCV_edf.csv");
//   for(std::size_t i = 0; i < GCV.values().size(); ++i) 
//     file_edf << std::setprecision(16) << std::sqrt(GCV.edfs()[i]) << "\n" ; 
...
//    GCV optimization: grid exact
//  */
// TEST(GCV_SQRPDE, Test1_Laplacian_NonParametric_GeostatisticalAtNodes_GridStoch) {
...
//    time penalization: separable (mass penalization)
//  */
// TEST(STQRPDE, Test2_Laplacian_SemiParametric_GeostatisticalAtLocations_Separable_Monolithic_GridExact) {
