...
TEST(GCV_SQRPDE, Test9_Laplacian_NonParametric_GeostatisticalAtNodes_GridExact) {
  ...
  for(double alpha : alphas){
    ...
    for(unsigned int m = 1; m <= M; ++m){
      ...
      std::string path = "/data/models/SQRPDE/Test_1/alpha_" + std::to_string(alpha_int) + "/sim_" 
                         + std::to_string(m); 
      ...
      std::ofstream fileLambdaopt(path + "/Lambda_exact.csv");
      if (fileLambdaopt.is_open()){
        fileLambdaopt << std::setprecision(16) << best_lambda[0];
        fileLambdaopt.close();
      }
...
TEST(GCV_SQRPDE, Test19_Laplacian_SemiParametric_GeostatisticalAtLocations_GridExact) {
  ...
  for(std::size_t m=7; m<=M; m++) {
    ...
    std::ofstream fileGCV_lambda(R_path + "/R/Our/data/Test_" + 
                TestNumber + "/alpha_" + alpha_string + "/sim_" + std::to_string(m) + "/GCV/Exact/GCV_lambdasCpp.csv");
    for(std::size_t i = 0; i < lambdas.size(); ++i) 
      fileGCV_lambda << std::setprecision(16) << lambdas[i] << "\n" ; 
    ...
    std::ofstream fileGCV_scores(R_path + "/R/Our/data/Test_" + 
                TestNumber + "/alpha_" + alpha_string + "/sim_" + std::to_string(m) + "/GCV/Exact/GCV_scoresCpp.csv");
    for(std::size_t i = 0; i < GCV.values().size(); ++i) 
      fileGCV_scores << std::setprecision(16) << std::sqrt(GCV.values()[i]) << "\n" ; 
...
TEST(GCV_SQRPDE, Test20_Laplacian_NonParametric_GeostatisticalAtNodes_GridExact) {
  ...
  //for(double x = -8.0; x <= -4.9; x +=0.2) lambdas.push_back(SVector<1>(std::pow(10,x)));  // alpha=10%
  //for(double x = -7.0; x <= -3.9; x +=0.2) lambdas.push_back(SVector<1>(std::pow(10,x)));  // alpha=50%
  for(double x = -8.0; x <= -4.9; x +=0.2) lambdas.push_back(SVector<1>(std::pow(10,x)));  // alpha=90%
 ...
 */
TEST(GCV_SQRPDE, Test26_Laplacian_SemiParametric_GeostatisticalAtMesh_GridExact) {
  // define domain and regularizing PDE
  ...
  double alpha = 0.9; 
  const std::string alpha_string = "90"; 
  ...
  // define grid of lambda values
  std::vector<SVector<1>> lambdas;
  for(double x = -6.4; x <= -4.9; x +=0.2) lambdas.push_back(SVector<1>(std::pow(10,x)));
...
/* test 18
   ...
   Correspondent in R: Test_2
 */
TEST(GCV_SQRPDE, Test18_Laplacian_SemiParametric_GeostatisticalAtNodes_GridStochastic) {
 ...
 */
TEST(GCV_SQRPDE, Test19_Laplacian_NonParametric_GeostatisticalAtNodes_GridExact) {
  ...
  std::vector<bool> lumping_vector = {false, true}; 
  unsigned int M = 5;   // number of simulations

  std::vector<SVector<1>> lambdasFALSE;
  for(double x = -6.0; x <= -5.3; x +=0.01) lambdasFALSE.push_back(SVector<1>(std::pow(10,x))); 
  std::vector<SVector<1>> lambdasTRUE;
  for(double x = -5.8; x <= -5.4; x +=0.01) lambdasTRUE.push_back(SVector<1>(std::pow(10,x)));  

  for(auto lump : lumping_vector){

    std::vector<SVector<1>> lambdas;
    std::string mass_type; 
    if(!lump){
      mass_type = "FALSE";
      for(auto l : lambdasFALSE)
        lambdas.push_back(l); 
    } 
    if(lump){
      mass_type = "TRUE";
      for(auto l : lambdasTRUE)
        lambdas.push_back(l); 
    }
      
    PDE problem(domain.mesh, L, u, lump); // definition of regularizing PDE
    // define the statistical model 
    SQRPDE<decltype(problem), fdaPDE::models::GeoStatLocations> model(problem, alpha);
    ...
    for(unsigned int m = 1; m <= M; ++m){
      ...
      std::string solution_path = R_path + "/R/Our/data/Test_" + 
                      "lumping" + "/alpha_" + alpha_string + "/lump" + mass_type + "/sim_" + std::to_string(m);
      ...
      // load locations where data are sampled
      CSVFile<double> locFile;
      locFile = reader.parseFile(R_path + "/R/Our/data/Test_lumping" +  "/alpha_" + alpha_string + 
                                "/locs.csv");
      DMatrix<double> loc = locFile.toEigen();
      model.set_spatial_locations(loc);
