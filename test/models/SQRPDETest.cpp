...
//   std::cout << "define domain" << std::endl ; 
//   MeshLoader<Mesh2D<>> domain("c_shaped_fine");         
//   std::cout << "fatto domain" << std::endl; 
//   auto L = Laplacian();
//   DMatrix<double> u = DMatrix<double>::Zero(domain.mesh.elements()*3, 1);
//   PDE problem(domain.mesh, L, u); // definition of regularizing PDE
...
//                         TestNumber + "/alpha_" + alpha_string + "/tol_weights_" + tol_weights_string + "/sim_M/n_" + seq_n_string[n] + "/LambdaCpp_" + alpha_string + ".csv");
...
//                 TestNumber + "/alpha_" + alpha_string + "/tol_weights_" + tol_weights_string + "/sim_M/n_" + seq_n_string[n] + "/sim_" + std::to_string(m) + "/fnCpp_" + alpha_string + ".csv");
...
//       const static Eigen::IOFormat CSVFormatf(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
//       std::ofstream filef(R_path + "/R/Our/data/Test_" + 
//                 TestNumber + "/alpha_" + alpha_string + "/tol_weights_" + tol_weights_string + "/sim_M/n_" + seq_n_string[n] + "/sim_" + std::to_string(m) + "/fCpp_" + alpha_string + ".csv");
...
//                 TestNumber + "/alpha_" + alpha_string + "/tol_weights_" + tol_weights_string +"/sim_M/n_" + seq_n_string[n] + "/sim_" + std::to_string(m) + "/betaCpp_" + alpha_string + ".csv");
...
//       // double J = model.J_final_sqrpde();
//       // std::ofstream fileJ(R_path + "/R/Our/data/Test_" + 
//       //             TestNumber + "/alpha_" + alpha_string + "/tol_weights_" + tol_weights_string + "/n_" + seq_n_string[n] + "/JCpp.csv");
//       // if (fileJ.is_open()){
//       //   fileJ << J;
//       //   fileJ.close();
//       // }
...
//       // std::size_t niter = model.niter_sqrpde();
//       // std::ofstream filen(R_path + "/R/Our/data/Test_" + 
//       //             TestNumber + "/alpha_" + alpha_string + "/tol_weights_" + tol_weights_string + "/n_" + seq_n_string[n] + "/niterCpp.csv");
//       // if (filen.is_open()){
//       //   filen << niter;
//       //   filen.close();
//       // }
...
TEST(SQRPDE, Test5_Laplacian_SemiParametric_GeostatisticalAtLocations) {
  ...
  std::vector<double> seq_tol_FPIRLS = {0.000001};   // , 0.0000001, 0.0000001};     // messa a 1e-7 
  ...
  unsigned int total_sim = 30;          // number of simulations 
  ...
  for (int k = 0; k < seq_n_string.size(); ++k){
    for(int i = 0; i < seq_tol_weights.size(); ++i ){
      for(int j = 0; j < seq_tol_FPIRLS.size(); ++j){
        ...
        //             TestNumber + "/alpha_" + alpha_string + "/" + data_macro_strategy_type + "/strategy_"  + data_strategy_type + 
        //             "/" + stopping_type + "/tol_weights_" + seq_tol_weights_string[i] + "/tol_FPIRLS_" + seq_tol_FPIRLS_string[j] + 
        //             "/" + lin_sys_solver + "/n_" + seq_n_string[k] + "/LambdaR_" + alpha_string + ".csv");    
        ...
        std::ofstream fileJ(R_path + "/R/Our/data/Test_" 
                    + TestNumber + "/alpha_" + alpha_string + "/" + data_macro_strategy_type + "/strategy_"  + data_strategy_type + 
                    "/" + stopping_type + "/tol_weights_" + seq_tol_weights_string[i] + "/tol_FPIRLS_" + seq_tol_FPIRLS_string[j] + 
                    "/" + lin_sys_solver + "/n_" + seq_n_string[k] + "/JCpp.csv");
        if (fileJ.is_open()){
          fileJ << J;
          fileJ.close();
        }
        ...
        std::ofstream filen(R_path + "/R/Our/data/Test_" 
                    + TestNumber + "/alpha_" + alpha_string + "/" + data_macro_strategy_type + "/strategy_"  + data_strategy_type + 
                    "/" + stopping_type + "/tol_weights_" + seq_tol_weights_string[i] + "/tol_FPIRLS_" + seq_tol_FPIRLS_string[j] + 
                    "/" + lin_sys_solver + "/n_" + seq_n_string[k] + "/niterCpp.csv");
        if (filen.is_open()){
          filen << niter;
          filen.close();
        }
